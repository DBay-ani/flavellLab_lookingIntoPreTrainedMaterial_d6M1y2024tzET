...
def default_conv(in_channels, out_channels, kernel_size, stride=(1, 1), bias=True):
    return nn.Conv2d(
        in_channels, out_channels, kernel_size=kernel_size,
        padding=(kernel_size // 2), stride=stride, bias=bias)


def default_conv3D(in_channels, out_channels, kernel_size, stride=(1, 1, 1), bias=True):
    return nn.Conv3d(
        in_channels, out_channels, kernel_size=kernel_size,
        padding=(kernel_size // 2), stride=stride, bias=bias)
...
class ResBlock(nn.Module):
    def __init__(
        self, conv=default_conv, n_feats=32, kernel_size=3, bias=True, bn=False, act=nn.PReLU(), res_scale=1):
