...
def sample_patches_from_multiple_stacksSR(datas, patch_size, HRpatch_size, n_samples, datas_mask=None,
                                          patch_filter=None):
    """ ((y, x), patch_size, HRpatch_size, n_patches_per_image, mask, patch_filter) """
    
    # TODO: some of these checks are already required in 'create_patches'
    len(patch_size) == datas[0].ndim or _raise(ValueError())
    
    if (not datas[0].shape == datas[0].shape) or (not datas[1].shape == datas[1].shape):
        raise ValueError("all input shapes must be the same: %s" % (" / ".join(str(a.shape) for a in datas)))
    ...
    if patch_filter is None:
        patch_mask = np.ones(datas[0].shape, dtype=np.bool)
        hrpatch_mask = np.ones(datas[1].shape, dtype=np.bool)
    else:
        patch_mask = patch_filter([datas[0]], patch_size)
        hrpatch_mask = patch_filter([datas[1]], HRpatch_size)
    ...
    border_slicesHR = tuple([slice(s // 2, d - s + s // 2 + 1) for s, d in zip(HRpatch_size, datas[1].shape)])
    valid_indsHR = np.where(hrpatch_mask[border_slicesHR])
    n_validHR = len(valid_indsHR[0])
    
    if n_valid == 0 or n_validHR == 0:
        raise ValueError("'patch_filter' didn't return any region to sample from")
    
    sample_inds = choice(range(n_valid), n_samples, replace=(n_valid < n_samples))
    rand_inds = [v[sample_inds] + s.start for s, v in zip(border_slices, valid_inds)]
    
    sample_indshr = choice(range(n_validHR), n_samples, replace=(n_validHR < n_samples))
    rand_indshr = [v[sample_indshr] + s.start for s, v in zip(border_slicesHR, valid_indsHR)]
    
    res = np.stack([datas[0][tuple(slice(_r - (_p // 2), _r + _p - (_p // 2)) for _r, _p in zip(r, patch_size))]
                    for r in zip(*rand_inds)])
    resHR = np.stack([datas[1][tuple(slice(_r - (_p // 2), _r + _p - (_p // 2)) for _r, _p in zip(r, HRpatch_size))]
                      for r in zip(*rand_indshr)])
...
def create_patchesSR(
    ...
    ## sample patches from each pair of transformed raw images
    X = np.empty((n_patches,) + tuple(patch_size), dtype=np.float32)
    Y = np.empty((n_patches,) + tuple(HRpatch_size), dtype=np.float32)
    ...
    for i, (x, y, _axes, mask) in tqdm(enumerate(image_pairs), total=n_images, disable=(not verbose)):
        ...
        _X, _Y = sample_patches_from_multiple_stacksSR((x, y), patch_size, HRpatch_size, n_patches_per_image,
                                                       datas_mask=None, patch_filter=None)
...
def create_img(
    ...
    for i, (x, y, _axes, mask) in tqdm(enumerate(image_pairs), total=n_images, disable=(not verbose)):
        ...
        # X, Y = normalization(_X,_Y, x,y,mask,channel)
